...
/*发送初始化*/
CAN_TxHeaderTypeDef Can_HandleTxMsg3;
...
Can_HandleTxMsg3.StdId=0x2FF;
Can_HandleTxMsg3.ExtId=0x00;
Can_HandleTxMsg3.IDE=CAN_ID_STD;
Can_HandleTxMsg3.RTR=CAN_RTR_DATA;
Can_HandleTxMsg3.DLC=8;
Can_HandleTxMsg3.TransmitGlobalTime=DISABLE;
}
...
uint8_t TX_MOTO_CURRENT3[8];//电机1-4号的速度(电流)数据处理
void Moto_Current_Output3(int16_t Current1,int16_t Current2,int16_t Current3,int16_t Current4)
{ 
	
 TX_MOTO_CURRENT3[0]=(uint8_t)(Current1>>8);
 TX_MOTO_CURRENT3[1]=(uint8_t)Current1;
	
 TX_MOTO_CURRENT3[2]=(uint8_t)(Current2>>8);
 TX_MOTO_CURRENT3[3]=(uint8_t)Current2;
	
 TX_MOTO_CURRENT3[4]=(uint8_t)(Current3>>8);
 TX_MOTO_CURRENT3[5]=(uint8_t)Current3;
	
 TX_MOTO_CURRENT3[6]=(uint8_t)(Current4>>8);
 TX_MOTO_CURRENT3[7]=(uint8_t)Current4;
	
 HAL_CAN_AddTxMessage(&hcan2,&Can_HandleTxMsg3,TX_MOTO_CURRENT3,0);
}	
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    case 0x203:
		...
		case 0x208:
		{
		 MotoData.yaw.Rx.angle=((uint16_t)RX_MOTO_DATA[0]<<8|(uint16_t)RX_MOTO_DATA[1]);
		 if(MotoData.yaw.Rx.flag==0)
		 {
		  MotoData.yaw.Tx.angle_target=MotoData.yaw.Rx.angle;
		 }
		 MotoData.yaw.Rx.flag=1;
		}break;
  }
...
if(hcan == &hcan2)//看看是不是can1的数据，后边电机用得多可能要开启can2，所以这里加一步用来辨别
	...
	switch(CAN_HandleRxMsg2.StdId)
	{
		...
		case 0x209:
		{
		 ...
		 MotoData.pitch.Rx.angle=((uint16_t)RX_MOTO_DATA2[0]<<8|(uint16_t)RX_MOTO_DATA2[1]);
		 if(MotoData.pitch.Rx.flag==0)
		 {
		  MotoData.pitch.Tx.angle_target=MotoData.pitch.Rx.angle;
		 }
		 MotoData.pitch.Rx.flag=1;
		}break;
	}
  }
